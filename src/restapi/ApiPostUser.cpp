...
StkObject* ApiPostUser::ExecuteImpl(StkObject* ReqObj, int Method, wchar_t UrlPath[StkWebAppExec::URL_PATH_LENGTH], int* ResultCode, wchar_t Locale[3], wchar_t* Token)
{
	...
	if (Id == -1 && StkPlWcsCmp(Password, L"") == 0) {
		AddCodeAndMsg(ResObj, MyMsgProc::CMDFRK_PASSWORD_REQUIRED_FOR_ADD, MyMsgProc::GetMsgEng(MyMsgProc::CMDFRK_PASSWORD_REQUIRED_FOR_ADD), MyMsgProc::GetMsgJpn(MyMsgProc::CMDFRK_PASSWORD_REQUIRED_FOR_ADD));
		*ResultCode = 400;
		return ResObj;
	}
	...
	*ResultCode = 200;
	if (Id == -1) {
		...
		wchar_t LogBufEng[512] = L"";
		wchar_t LogBufJpn[512] = L"";
		StkPlSwPrintf(LogBufEng, 512, MyMsgProc::GetMsgEng(MyMsgProc::CMDFRK_USER_EDIT), TmpName);
		StkPlSwPrintf(LogBufJpn, 512, MyMsgProc::GetMsgJpn(MyMsgProc::CMDFRK_USER_EDIT), TmpName);
		DataAccess::GetInstance()->AddLogMsg(LogBufEng, LogBufJpn);
	}
