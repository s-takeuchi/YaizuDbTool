...
PostgreSqlAccessor::PostgreSqlAccessor()
{
}
...
void PostgreSqlAccessor::GetDefaultConnStr(SQLTCHAR DefConnStr[Global::MAX_PARAM_LENGTH])
{
	lstrcpy(DefConnStr, _T("Driver={PostgreSQL Unicode};Server=127.0.0.1;Database=DATABASE_NAME;UID=UID;PWD=PWD;Port=5432;"));
}

int PostgreSqlAccessor::GetNumOfRecords(SQLTCHAR* TableName, SQLTCHAR StateMsg[10], SQLTCHAR* Msg, SQLSMALLINT MsgLen)
{
...
SQLRETURN PostgreSqlAccessor::GetTables(SQLTCHAR* OutJson, int SizeOfOutJson, SQLTCHAR StateMsg[10], SQLTCHAR* Msg, SQLSMALLINT MsgLen)
{
	SQLRETURN Ret = 0;
	...
	Ret = GetTablesCommon(_T("select relname as TABLE_NAME from pg_stat_user_tables;"), OutJson, SizeOfOutJson, StateMsg, Msg, MsgLen);
	Ret = CloseDatabase(StateMsg, Msg, MsgLen);

	return Ret;
}

int PostgreSqlAccessor::GetColumnInfoByTableName(SQLTCHAR* TableName,
...
int PostgreSqlAccessor::GetRecordsByTableName(SQLTCHAR* TableName,
	int NumOfCols,
...
int PostgreSqlAccessor::ConvertAttrType(SQLTCHAR InAttr[Global::COLUMNTYPE_LENGTH], SQLTCHAR OutAttr[Global::COLUMNTYPE_LENGTH])
{
...
void PostgreSqlAccessor::SqlEncoding(SQLTCHAR* InSql, SQLTCHAR* OutSql, int Type)
{
