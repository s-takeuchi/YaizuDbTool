	...
	SQLTCHAR StateMsg[10], SQLTCHAR* Msg, SQLSMALLINT MsgLen)
{
	...
	for (;;) {
		...
		for (int LoopCol = 0; LoopCol < NumOfCols; LoopCol++) {
			if (ValLen[LoopCol] == -1) {
				lstrcpyn(&Record[LoopRec * Global::MAXNUM_COLUMNS * Global::COLUMNVAL_LENGTH + LoopCol * Global::COLUMNVAL_LENGTH], _T(""), Global::COLUMNVAL_LENGTH);
			} else {
				lstrcpyn(&Record[LoopRec * Global::MAXNUM_COLUMNS * Global::COLUMNVAL_LENGTH + LoopCol * Global::COLUMNVAL_LENGTH], TmpRecord[LoopCol], Global::COLUMNVAL_LENGTH);
			}
		}
		LoopRec++;
